...
const TABLES = [
  { id: 'premiere', name: 'Premiere', sub: 'Ruleta Europea Clásica', tag: 'LIVE', tagColor: GOLD, minBet: 100, maxBet: 15000, players: 142, gradient: 'linear-gradient(160deg, #1a0800 0%, #0a0a0a 100%)', vip: false },
  { id: 'velocity', name: 'Midnight Velocity', sub: 'Rondas rápidas 20s', tag: 'FAST PLAY', tagColor: '#4B9CD3', minBet: 100, maxBet: 5000, players: 589, gradient: 'linear-gradient(160deg, #000a1a 0%, #0a0a0a 100%)', vip: false },
  { id: 'vip', name: 'VIP Lounge', sub: 'Dealer Personal', tag: 'HIGH STAKES', tagColor: GOLD, minBet: 5000, maxBet: 100000, players: 8, gradient: 'linear-gradient(160deg, #0d0a00 0%, #1f1700 60%, #0a0a0a 100%)', vip: true },
]
    ...
    <>
      ...
      <main style={{ minHeight: '100dvh', background: DARK, fontFamily: "'Montserrat', sans-serif", maxWidth: '480px', margin: '0 auto', paddingBottom: '40px' }}>
        ...
        <div style={{ position: 'relative', margin: '20px', borderRadius: '8px', padding: '32px 24px', background: 'linear-gradient(160deg, #1a0800 0%, #0a0a0a 100%)', border: '1px solid rgba(212,175,55,0.15)' }}>
          ...
          <p style={{ fontSize: '0.55rem', color: 'rgba(255,255,255,0.35)', letterSpacing: '0.08em', lineHeight: 1.7 }}>Experimentá la emoción de la ruleta en nuestras suites de alto límite.</p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', padding: '0 20px', marginBottom: '24px' }}>
          ...
          <div style={{ background: '#111', border: '1px solid rgba(212,175,55,0.1)', borderRadius: '6px', padding: '16px' }}>
            <p style={{ fontSize: '0.45rem', letterSpacing: '0.25em', color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', marginBottom: '6px' }}>Mesas Activas</p>
            <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '2rem', color: '#fff', fontWeight: 300 }}>24 <span style={{ fontSize: '0.5rem', color: GOLD, letterSpacing: '0.2em' }}>LIVE</span></p>
          </div>
        </div>

        <div style={{ padding: '0 20px' }}>
          <p style={{ fontSize: '0.5rem', letterSpacing: '0.3em', color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', marginBottom: '16px' }}>Seleccioná tu mesa</p>
          {TABLES.map(table => (
              ...
              <div style={{ padding: '20px' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: '14px' }}>
                  <div>
                    ...
                    <p style={{ fontSize: '0.7rem', color: GOLD, fontWeight: 700 }}>{table.minBet.toLocaleString('es-UY')} — {table.maxBet.toLocaleString('es-UY')} CHIPS</p>
                  </div>
                  <button className="join-btn" onClick={() => router.push('/roulette/play')} style={{ background: table.vip ? GOLD : 'rgba(212,175,55,0.12)', border: \`1px solid \${table.vip ? GOLD : 'rgba(212,175,55,0.3)'}\`, borderRadius: '3px', padding: '10px 20px', fontFamily: "'Montserrat', sans-serif", fontWeight: 700, fontSize: '0.55rem', letterSpacing: '0.25em', color: table.vip ? '#000' : GOLD, textTransform: 'uppercase' }}>
                    {table.vip ? 'SOLICITAR' : 'JUGAR'}
                  </button>
                </div>
              </div>
            </div>
        ...
        <div style={{ margin: '24px 20px 0', padding: '24px', background: '#0d0d0d', border: '1px solid rgba(212,175,55,0.1)', borderRadius: '6px', textAlign: 'center' }}>
          <p style={{ fontSize: '0.45rem', letterSpacing: '0.3em', color: GOLD, textTransform: 'uppercase', marginBottom: '8px' }}>Promoción activa</p>
          <h3 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '1.4rem', color: '#fff', fontWeight: 300, marginBottom: '10px' }}>Golden Spin Weekend</h3>
          <p style={{ fontSize: '0.55rem', color: 'rgba(255,255,255,0.3)', lineHeight: 1.7, marginBottom: '16px' }}>Doble de rewards en todas las mesas VIP este fin de semana.</p>
          <button style={{ background: 'transparent', border: '1px solid rgba(212,175,55,0.3)', borderRadius: '3px', padding: '10px 24px', fontFamily: "'Montserrat', sans-serif", fontWeight: 700, fontSize: '0.55rem', letterSpacing: '0.25em', color: GOLD, cursor: 'pointer' }}>VER MÁS</button>
        </div>
...
// Crear carpeta play
fs.mkdirSync('src/app/roulette/play', { recursive: true });

// Mover el juego actual a /play
if (fs.existsSync('src/app/roulette/page.tsx')) {
  const gameContent = fs.readFileSync('src/app/roulette/page.tsx', 'utf8');
  fs.writeFileSync('src/app/roulette/play/page.tsx', gameContent, 'utf8');
  console.log('✅ Juego movido a /roulette/play/page.tsx');
}
