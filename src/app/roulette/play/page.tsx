    ...
    <>
      ...
      <main style={{ minHeight: '100dvh', background: DARK, fontFamily: "'Montserrat', sans-serif", maxWidth: '480px', margin: '0 auto', paddingBottom: '80px' }}>
        ...
        <div style={{ background: 'rgba(10,10,10,0.95)', position: 'sticky', top: 0, zIndex: 90, borderBottom: '1px solid rgba(212,175,55,0.2)', padding: '7px 16px', display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <span style={{ fontSize: '0.68rem', color: GOLD, fontWeight: 500, flexShrink: 0 }}>SOPHIE</span>
          {/* Caja */}
          <button onClick={() => setShowPayment(true)} style={{ background: 'linear-gradient(180deg,#2d7a4f 0%,#1e5c38 100%)', border: 'none', borderRadius: 4, padding: '4px 12px', fontSize: '0.68rem', color: '#fff', fontWeight: 700, cursor: 'pointer', flexShrink: 0 }}>Caja</button>
          {/* Selector moneda */}
          ...
          <span style={{ display: 'flex', alignItems: 'center', gap: '4px', marginLeft: 'auto', flexShrink: 0 }}>
            ...
            <span style={{ fontSize: '0.68rem', color: 'rgba(255,255,255,0.5)' }}>{username}</span>
          </span>
        </div>
