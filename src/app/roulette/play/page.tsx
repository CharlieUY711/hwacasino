      ...
      <main style={{ minHeight: '100dvh', background: DARK, fontFamily: "'Inter', sans-serif", maxWidth: '480px', margin: '0 auto', paddingBottom: '80px' }}>
        ...
        <div style={{ background: 'rgba(10,10,10,0.95)', position: 'sticky', top: 0, zIndex: 90 }}>
          ...
          <div style={{ padding: '6px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '8px' }}>
            ...
            {/* Centro: boton Caja doble ancho */}
            <button onClick={() => setShowPayment(true)} style={{ background: 'linear-gradient(180deg,#4ade80 0%,#16a34a 50%,#15803d 100%)', border: 'none', borderBottom: '2px solid #166534', borderRadius: 4, padding: '6px 32px', fontSize: '0.72rem', color: '#fff', fontWeight: 700, cursor: 'pointer', flex: 2, maxWidth: 140 }}>
              Caja
            </button>
            {/* Derecha: usuario + balance */}
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', flex: 1 }}>
              <span style={{ fontSize: '0.65rem', color: 'rgba(255,255,255,0.45)', fontWeight: 500 }}>{username}</span>
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                {showPayout ? (
                  ...
                  <span style={{ fontSize: '0.65rem', color: '#f87171' }}>
                    -{hasBetThisRound ? lastBets.reduce((s,b)=>s+b.amount,0).toLocaleString('es-UY') : totalBet.toLocaleString('es-UY')}
                  </span>
                ) : null}
          ...
          {/* Linea divisoria + flecha abajo izquierda */}
          <div style={{ borderTop: '1px solid rgba(212,175,55,0.12)', padding: '2px 16px 3px' }}>
            <button onClick={() => router.push('/roulette')} style={{ background: 'transparent', border: 'none', color: 'rgba(255,255,255,0.35)', cursor: 'pointer', fontSize: '0.9rem', padding: 0, lineHeight: 1 }}>
              ←
            </button>
          </div>
        </div>
